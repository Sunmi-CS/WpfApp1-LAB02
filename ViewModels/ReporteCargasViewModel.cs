...
{
    public class ReporteCargasViewModel : BaseViewModel
    {
        ...
        public ObservableCollection<Carga> Resultados { get; } = new ObservableCollection<Carga>();
        ...
        public ReporteCargasViewModel(AppDataService service)
        {
            _service = service ?? throw new ArgumentNullException(nameof(service));
            BuscarCommand = new RelayCommand(_ => Buscar());
            LimpiarCommand = new RelayCommand(_ => Limpiar());
            Refrescar(_service.Cargas);
        }

        private void Refrescar(System.Collections.Generic.IEnumerable<Carga> items)
        {
            Resultados.Clear();
        ...
        private void Buscar()
        {
            var q = _service.FilterCargas(FechaInicio, FechaFin, Placa, Conductor, Producto);
            Refrescar(q);
        }
